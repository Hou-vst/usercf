 ...
 void Create_UserToItem_Table(map<string, set<string>>& map)
{
 ...
 //	引入惩罚机制来消减热门物品对相似度的影响，建立稀疏矩阵
 void Create_CoRated_table_improve(map<string, set<string>>& dst, map<string, map<string, float>>& CoRated_table)
 {
	 //遍历所有物品
	 map<string, set<string>>::iterator iter_begin = dst.begin();
	 map<string, set<string>>::iterator iter_End = dst.end();
	 for (; iter_begin != iter_End; iter_begin++)
	 {
		 ...
		 for (; set_begin1 != user_set.end(); set_begin1++)
		 {
			 ...
			 for (; set_begin2 != user_set.end(); set_begin2++)
			 {
				 ...
				 if (!init)
				 {
					 CoRated_table[user1][user2] = 1/log(1+user_set_size);
				 }
				 else
				 {
					 CoRated_table[user1][user2]+= 1 / log(1 + user_set_size);
 ...
 // 计算用户与用户之间的相似度
 void Calculate_Similarity(const map<string, map<string, float>>& CoRated_table, map<string, set<string>>& user_to_item,map<string, map<string, float>>& result)
 {
 ...
 int main()
 {
	 ...
	 cout << Calculate(v, k) << endl <<endl;

	 cout << "引入对热门商品的惩罚机制:" << endl;

	 CoRated_table.clear();
	 result.clear();
	 Create_CoRated_table_improve(item_to_user, CoRated_table);
	 Calculate_Similarity(CoRated_table, user_to_item, result);
	 PrintResult(result);
	 cout << "计算用户user :" << user.c_str() << "对物品item :" << item.c_str() << "的感兴趣程度,k值为" << k << endl;
	 v= SortSimilarUser(result, user, item, item_to_user);
